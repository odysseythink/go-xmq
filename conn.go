...
// returned from an IDENTIFY command to xmqd
...
// returned from an AUTH command to xmqd
...
// Conn represents a connection to xmqd
//
...
// Connect dials and bootstraps the xmqd connection
...
// MaxRDY returns the xmqd negotiated maximum
...
// RemoteAddr returns the configured destination xmqd address
...
func (c *Conn) writeLoop() {
	for {
		...
		case resp := <-c.msgResponseChan:
			// Decrement this here so it is correct even if we can't respond to xmqd
...
func (c *Conn) close() {
	...
	//
	//     1. CLOSE cmd sent to xmqd
	//     2. CLOSE_WAIT response received from xmqd
	//     3. set c.closeFlag
	//     4. readLoop() exits
