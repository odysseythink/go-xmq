...
// Producer is a high-level type to publish to XMQ.
//
// A Producer instance is 1:1 with a destination `xmqd`
...
// Ping causes the Producer to connect to it's configured xmqd (if not already
...
// but does not wait for the response from `xmqd`.
//
// When the Producer eventually receives the response from `xmqd`,
...
func (w *Producer) connect() error {
	...
	w.log(LogLevelInfo, "(%s) connecting to xmqd", w.addr)
	...
	if err != nil {
		w.conn.Close()
		w.log(LogLevelError, "(%s) error connecting to xmqd - %s", w.addr, err)
		return err
	}
