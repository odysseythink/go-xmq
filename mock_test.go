...
func newMockXMQD(t *testing.T, script []instruction, addr string) *mockXMQD {
	n := &mockXMQD{
		t:        t,
		script:   script,
...
func (n *mockXMQD) handle(conn net.Conn) {
	var idx int
...
func TestConsumerBackoff(t *testing.T) {
	...
	n := newMockXMQD(t, script, addr.String())
	...
	err := q.ConnectToXMQD(n.tcpAddr.String())
	if err != nil {
		t.Fatalf(err.Error())
	}
...
func TestConsumerBackoffDisconnect(t *testing.T) {
	...
	n = newMockXMQD(t, script, n.tcpAddr.String())
