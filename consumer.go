...
//
//	consumer.AddHandler(xmq.HandlerFunc(func(m *Message) error {
//		// handle the message
//	}))
...
// for filtering the xmqds returned from discovery via xmqlookupd
...
// Consumer is a high-level type to consume from XMQ.
//
...
//
// If configured, it will poll xmqlookupd instances and handle connection (and
// reconnection) to any discovered xmqds.
...
//
// This may change dynamically based on the number of connections to xmqd the Consumer
// is responsible for.
...
// ConnectToXMQLookupd adds an xmqlookupd address to the list for this Consumer instance.
//
// If it is the first to be added, it initiates an HTTP request to discover xmqd
// producers for the configured topic.
//
// A goroutine is spawned to handle continual polling.
func (r *Consumer) ConnectToXMQLookupd(addr string) error {
...
// ConnectToXMQLookupds adds multiple xmqlookupd address to the list for this Consumer instance.
//
// If adding the first address it initiates an HTTP request to discover xmqd
// producers for the configured topic.
//
// A goroutine is spawned to handle continual polling.
func (r *Consumer) ConnectToXMQLookupds(addresses []string) error {
	for _, addr := range addresses {
		err := r.ConnectToXMQLookupd(addr)
		if err != nil {
			return err
		}
	}
...
// make an HTTP req to one of the configured xmqlookupd instances to discover
// which xmqd's provide the topic we are consuming.
//
...
retry:
	...
	r.log(LogLevelInfo, "querying xmqlookupd %s", endpoint)
	...
	if err != nil {
		r.log(LogLevelError, "error querying xmqlookupd (%s) - %s", endpoint, err)
		retries++
		if retries < 3 {
			r.log(LogLevelInfo, "retrying with next xmqlookupd")
			goto retry
		}
		return
	}

	var xmqdAddrs []string
	for _, producer := range data.Producers {
		broadcastAddress := producer.BroadcastAddress
		port := producer.TCPPort
		joined := net.JoinHostPort(broadcastAddress, strconv.Itoa(port))
		xmqdAddrs = append(xmqdAddrs, joined)
	}
	// apply filter
	if discoveryFilter, ok := r.behaviorDelegate.(DiscoveryFilter); ok {
		xmqdAddrs = discoveryFilter.Filter(xmqdAddrs)
	}
	for _, addr := range xmqdAddrs {
		err = r.ConnectToXMQD(addr)
		if err != nil && err != ErrAlreadyConnected {
			r.log(LogLevelError, "(%s) error connecting to xmqd - %s", addr, err)
			continue
		}
	}
}

// ConnectToXMQDs takes multiple xmqd addresses to connect directly to.
//
// It is recommended to use ConnectToXMQLookupd so that topics are discovered
// automatically.  This method is useful when you want to connect to local instance.
func (r *Consumer) ConnectToXMQDs(addresses []string) error {
	for _, addr := range addresses {
		err := r.ConnectToXMQD(addr)
		if err != nil {
			return err
...
// ConnectToXMQD takes a xmqd address to connect directly to.
//
// It is recommended to use ConnectToXMQLookupd so that topics are discovered
// automatically.  This method is useful when you want to connect to a single, local,
// instance.
func (r *Consumer) ConnectToXMQD(addr string) error {
	if atomic.LoadInt32(&r.stopFlag) == 1 {
		return errors.New("consumer stopped")
	}
	...
	if idx := indexOf(addr, r.xmqdTCPAddrs); idx == -1 {
		r.xmqdTCPAddrs = append(r.xmqdTCPAddrs, addr)
	}
	r.mtx.Unlock()

	r.log(LogLevelInfo, "(%s) connecting to xmqd", addr)
...
// DisconnectFromXMQD closes the connection to and removes the specified
// `xmqd` address from the list
func (r *Consumer) DisconnectFromXMQD(addr string) error {
	r.mtx.Lock()
	defer r.mtx.Unlock()

	idx := indexOf(addr, r.xmqdTCPAddrs)
	if idx == -1 {
		return ErrNotConnected
	}

	// slice delete
	r.xmqdTCPAddrs = append(r.xmqdTCPAddrs[:idx], r.xmqdTCPAddrs[idx+1:]...)
...
// DisconnectFromXMQLookupd removes the specified `xmqlookupd` address
// from the list used for periodic discovery.
func (r *Consumer) DisconnectFromXMQLookupd(addr string) error {
	parsedAddr, err := buildLookupAddr(addr, r.topic)
	if err != nil {
		return err
	}
	...
	if len(r.lookupdHTTPAddrs) == 1 {
		return fmt.Errorf("cannot disconnect from only remaining xmqlookupd HTTP address %s", addr)
	}
...
func (r *Consumer) onConnResponse(c *Conn, data []byte) {
	...
	case bytes.Equal(data, []byte("CLOSE_WAIT")):
		...
		r.log(LogLevelInfo, "(%s) received CLOSE_WAIT from xmqd", c.String())
		c.Close()
	}
}
...
func (r *Consumer) onConnClose(c *Conn) {
	...
	r.mtx.RLock()
	numLookupd := len(r.lookupdHTTPAddrs)
	reconnect := indexOf(c.String(), r.xmqdTCPAddrs) >= 0
	r.mtx.RUnlock()
	if numLookupd > 0 {
		...
		// there are no lookupd and we still have this xmqd TCP address in our list...
		...
		go func(addr string) {
			for {
				...
				r.mtx.RLock()
				reconnect := indexOf(addr, r.xmqdTCPAddrs) >= 0
				r.mtx.RUnlock()
				if !reconnect {
...
func (r *Consumer) updateRDY(c *Conn, count int64) error {
	...
	// never exceed the xmqd's configured max RDY count
	if count > c.MaxRDY() {
		count = c.MaxRDY()
	}
...
//
// # This panics if called after connecting to XMQD or XMQ Lookupd
//
