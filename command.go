...
// Command represents a command from a client to an xmq daemon
type Command struct {
	Name   []byte
	Params [][]byte
	Body   []byte
...
// Register creates a new Command to add a topic/channel for the connected xmqd
...
// UnRegister creates a new Command to remove a topic/channel for the connected xmqd
...
func Ping(args ...string) *Command {
	...
	return &Command{[]byte("PING"), params, nil}
}
...
// client would like to start a close cycle.  xmqd will no longer
