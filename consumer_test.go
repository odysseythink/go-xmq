...
func TestConsumerLookupdAuthorization(t *testing.T) {
	...
	q.ConnectToXMQLookupd(lookupd.URL)
	if req == false {
...
func consumerTest(t *testing.T, cb func(c *Config)) {
	...
	err = q.ConnectToXMQD(addr)
	if err == nil {
		t.Fatal("should not be able to connect to the same XMQ twice")
	}
	...
	err = q.DisconnectFromXMQD("1.2.3.4:4150")
	if err == nil {
		t.Fatal("should not be able to disconnect from an unknown xmqd")
	}

	err = q.ConnectToXMQD("1.2.3.4:4150")
	if err == nil {
		t.Fatal("should not be able to connect to non-existent xmqd")
	}

	err = q.DisconnectFromXMQD("1.2.3.4:4150")
	if err != nil {
		t.Fatal("should be able to disconnect from an xmqd - " + err.Error())
	}
